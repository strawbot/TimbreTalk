	...
	def fileOperation(self, operation):
		try:
			...
			if self.sendState == IDLE:
				...
				self.protocol.setHandler(pids.ERASE_CONF, self.eraseConfirmed)
				self.protocol.setHandler(pids.WRITE_CONF, self.transferConfirmed)
				self.protocol.setHandler(pids.MEM_CHECK, self.verifyConfirmed)
				self.starting.emit()
				operation()
